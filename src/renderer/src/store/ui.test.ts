...
describe('侧栏分组折叠', () => {
  it('按 id 记，互不影响，再点一下展开', () => {
    useUI.getState().toggleGroup('dms')
    expect(useUI.getState().collapsed.dms).toBe(true)
    expect(useUI.getState().collapsed.channels).toBeUndefined()
    useUI.getState().toggleGroup('dms')
    expect(useUI.getState().collapsed.dms).toBe(false)
  })
})
