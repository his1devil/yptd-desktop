...
import { ChannelBots } from '../views/AgentConfig'
 ...
 * 显示"左宽右窄"的分栏示意和三条提示。
 */
const BASE_TABS: Record<string, string> = { members: '成员', bots: '机器人', settings: '设置', runs: '运行', profile: '资料' }
...
export function Inspector() {
  ...
  const base = !place ? [] : place.kind === 'channel' ? ['members', 'bots', 'settings', 'runs'] : place.isAgent ? ['profile', 'runs'] : []
    ...
    <aside className={`${styles.side} ${shown ? '' : styles.shut}`} style={{ width: shown ? width : 0 }} aria-hidden={!shown}>
      ...
      <div className={styles.body}>
        {current === 'members' && place ? (
          <Members place={place} />
        ) : current === 'bots' && place?.groupID ? (
          <ChannelBots groupID={place.groupID} members={place.members} />
