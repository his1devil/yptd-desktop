...
## 8. 第二批修复：图片资源与加载状态（F5 / F6 / F7）

### F5 按槽位取图

对象存储认 `?type=image&width=W&height=H`，对原生图片消息和自有上传的附件都生效；只给宽高不带 `type` 会返回原图，请求的尺寸大于原图时也返回原图，因此不会被放大。

实现放在渲染层（`im/files.ts` 的 `sized()`），模型不动：消息里存的仍是原图地址，列表按槽位乘屏幕像素密度、归到 160 的整数倍去取，灯箱取原图。这样历史消息不用迁移，也不需要额外上传缩略图。

实测（2400×2400 的照片，显示在 200×200 的槽位里）：

| | 字节 | 耗时 |
| --- | --- | --- |
| 原图 | 638701 | 1869ms |
| 列表用的 480px | 226971 | 930ms |

### F7 估高与渲染共用一份排布

`views/gallery.ts` 的 `galleryLayout()` 同时供估高和渲染调用，并按消息区实际可用宽度计算（右栏开合、窗口缩放会改变它，`ResizeObserver` 跟着更新），不再假设固定 760 和"一行四张"。实测画廊行数与高度和渲染结果完全一致。

### F6 三个阶段分开表达

新增 `Shot` 组件统一所有图片槽位：

- 尺寸在图到之前就定下来，图加载完不推动列表。
- 上传中：本机缩略图 + 一道扫光。SDK 不提供字节进度，所以不显示百分比。
- 消息确认后扫光撤掉；清晰图解码完才淡入，本机缩略图在底下垫着，不闪。
- 裁过的地址取不到自动退回原图，再取不到就地显示「点一下重试」。

附件上传从串行改为三个并发，保持栏里的顺序；本机预览缓存加了 60 张上限。

## 9. 第三批修复：滚动补偿与图片查看器（F4 / F3）

### F4 滚动补偿交还给库

删掉 `shouldAdjustScrollPositionOnItemSizeChange` 的覆盖即可。virtual-core 3.17.9 的默认判断分两种情况：
第一次量高按「行首在视口之上」补（估高换成真高，整块都在上面）；重新量高只补「整行都在视口之上」的，
并且向上滚时不补。原来的覆盖只看行首，所以一条横跨视口顶部的长消息在底部增长时也会被补，把正在读的内容往上推。

视口高度变化也纳入贴底：输入框打多行会压矮消息区，内容高度没变但视口变了，`ResizeObserver` 里补一次贴底。
实测打五行时视口从 664 降到 576，消息流仍然贴着底。

顺带修掉一个 0.3.4 的回归：输入框的自适应高度是靠「先设成 0 再读 scrollHeight」量的，
0.3.4 给它加了高度过渡之后，设成 0 不会立刻生效，量到的是动画中途那个还很高的盒子，
于是输入框只长不缩——多行消息发出去之后再也收不回来。量之前把过渡关掉即可。
实测一行 46、五行 134、回到两行 68、清空 46、超过八行封顶 200。

### F3 图片查看器改成真正的模态

没有引第三方库。评估下来现在缺的是关闭按钮、焦点约束和左右切图，自己补比接一个带自己滚动锁和
快捷键的库更省；缩放拖动等真需要时再谈。

- 用 portal 送到 `document.body`，不受消息区布局、层叠和祖先 transform 影响。
- `role="dialog"`、`aria-modal="true"`、显式关闭按钮。
- 焦点进来后限制在层内（Tab 到末尾回到首个，Shift+Tab 反向），关掉把焦点还回原来的缩略图。
- 打开期间应用级快捷键让位，`App.tsx` 认 `data-modal`。实测 ⌘K 不开命令面板、⌥↓ 不切会话。
- 同一条消息里的图成一组，左右方向键切换并显示「2 / 3」计数。
- 关闭时按当前这张图现找缩略图做缩回动画，中间翻过好几张或原缩略图已被虚拟列表回收都不会缩错地方。

实测：挂在 BODY 下、遮罩铺满整窗、焦点不外泄、Esc 关闭后焦点回到缩略图、遮罩实际像素 (55,55,56)。

### 仍未做

灯箱的缩放与拖动、把查看范围从「当前消息」扩到「当前会话已加载的图」、审查第 3 节提到的
上传失败就地重试（现在是退回输入框重发）。
